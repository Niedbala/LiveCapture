...
namespace SmoReader
{
    public class FileReader
    {
        ...
        {
            _loggingService?.Log($"configDate: {configDate.ToString()}");
        ...
        private FileReadResult ReadFileBase(string filepath, string aircraftName, DateTime? configDate, bool isSinglePacketRead = false)
        {
            ...
            try
            {
                ...
                var packetReader = new PacketDecoder(parameterDefinitions, _aircraftDataProvider);
                ...
                readResult.AcraTime = null;
                readResult.EthernetTime = null;

                try
                {
                    if (acraTimeSignatures != null && acraTimeSignatures.Any())
                ...
                try
                {
                    if (ethernetTimeSignatures != null && ethernetTimeSignatures.Any())
            ...
            catch (Exception e)
            {
                if (e is AccessViolationException)
                {
                    ...
                    var numExtractedSamples = extractedSamples.FirstOrDefault().Value.Count;

                    _loggingService?.Log($"Extracted: {numExtractedSamples} samples", LogLevel.Warning);
                    _loggingService?.Log(e.Message);
                }
                else
                {
                    flags.GeneralReadError = true;
